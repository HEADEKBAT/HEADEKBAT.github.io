...
function firstChangeText() {
    changeText.innerHTML = "$278.000"
}

function changeColorText() {
    changeText.classList.add('change-color')
}

function addPulse() {
    const pulseButtons = document.querySelectorAll('.form-button')
    for (const pulseButton of pulseButtons) {
        pulseButton.classList.add('pulse')
    }
}

function remuveColorPulse() {
    const checkboxSales = document.querySelectorAll('.checkbox__sale')
    for (const checkboxSale of checkboxSales) {
        checkboxSale.classList.remove('pulse-color')
    }
}

function changePrice() {
    // Проверяем наличие элемента 'number' на странице
    const numberElement = document.getElementById('number');
    if (!numberElement) {
        console.error("Элемент 'number' не найден на странице.");
    } else {
        // Устанавливаем начальное число и конечное число
        let startNumber = 278;
        const endNumber = 139;

        // Указываем время, за которое нужно достичь конечного числа (в миллисекундах)
        const duration = 1000;

        // Указываем интервал обновления числа (в миллисекундах)
        const updateInterval = 50;

        // Вычисляем разницу между начальным и конечным числами
        const difference = startNumber - endNumber;

        // Вычисляем шаг изменения числа на каждом интервале
        const step = difference / (duration / updateInterval);

        // Функция для обновления числа
        function updateNumber() {
            startNumber -= step;
            numberElement.innerText = "$" + Math.round(startNumber) + '.000';
        ...
        // Запускаем интервал обновления числа
        const intervalId = setInterval(updateNumber, updateInterval);
    }
    for (const oldPriceItem of oldPrice) {
        oldPriceItem.classList.add('bef')
    }
}
